...
2. Refactor frontend around domains/workflows.
3. Split backend responsibilities into routes, job store, storage, worker dispatch, and pipeline modules.
4. Move Cadence behind a server-owned backend boundary.
5. Normalize progression data so Journey is derived from evidence rather than page-level heuristics.
